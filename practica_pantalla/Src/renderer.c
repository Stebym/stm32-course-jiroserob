...
void Renderer_DrawSplash(void) {
    ...
    ILI9341_FillRect(20, 72, 280, 3, COLOR_WHITE);

    /* Icono GUITARRA (x~30-70, y~78-135): mastil + clavijero + cuerpo + cuerdas */
    ILI9341_FillRect(42, 78, 18, 7, COLOR_DARKGRAY);
    ILI9341_FillRect(48, 85, 6, 25, COLOR_GRAY);
    ILI9341_FillCircle(51, 118, 17, COLOR_ORANGE);
    ILI9341_FillCircle(51, 118, 5, COLOR_BLACK);
    ILI9341_DrawLine(48, 78, 48, 133, COLOR_WHITE);
    ILI9341_DrawLine(54, 78, 54, 133, COLOR_WHITE);

    /* Icono SIMON DICE (x=140-180, y=95-135): cuadricula 2x2 de colores */
    ILI9341_FillRect(140, 95,  18, 18, COLOR_RED);
    ILI9341_FillRect(162, 95,  18, 18, COLOR_GREEN);
    ILI9341_FillRect(140, 117, 18, 18, COLOR_BLUE);
    ILI9341_FillRect(162, 117, 18, 18, COLOR_YELLOW);

    /* Icono BUZZER (x~250-290, y~100-134): parlante + ondas de sonido */
    ILI9341_FillRect(250, 106, 12, 24, COLOR_GRAY);
    ILI9341_FillCircle(262, 118, 14, COLOR_GRAY);
    ILI9341_DrawLine(276, 110, 286, 102, COLOR_CYAN);
    ILI9341_DrawLine(276, 118, 290, 118, COLOR_CYAN);
    ILI9341_DrawLine(276, 126, 286, 134, COLOR_CYAN);

    /* Mensaje de bienvenida */
    draw_string_c(LCD_W / 2, 145, "BIENVENIDOS", COLOR_WHITE, COLOR_BLACK, 2);

    /* Credito del creador */
    draw_string_c(LCD_W / 2, 188, "POR: JIMMY STEBYM ROSERO BARRERA",
                  COLOR_GRAY, COLOR_BLACK, 1);

    /* Subtitulo parpadeante "LISTO PARA JUGAR?" — dibujado por Renderer_Update */
...
void Renderer_DrawSeleccionModo(uint8_t cursor) {
    ...
    static const uint16_t    card_col[3]  = { COLOR_RED, COLOR_MAGENTA, COLOR_YELLOW };
    static const char *const card_name[3] = { "SIMON", "SIM+JOY", "GT HERO" };
    static const char *const card_desc[3] = {
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        uint16_t name_w = str_pixel_w(card_name[i], 1);
        uint16_t name_x = bx + (bw - name_w) / 2;
        draw_string(name_x, by + 62, card_name[i], card_col[i], inner_bg, 1);
...
static void draw_player_topbar(uint16_t xo, uint16_t player_color, const char *label) {
    ILI9341_FillRect(xo, 0, PLAYER_W, SCORE_BAR_H, COLOR_DARKGRAY);
    ILI9341_FillRect(xo + 1, 2, 20, 20, player_color);
    draw_string(xo + 5, 7, label, COLOR_WHITE, player_color, 1);
}
...
void Renderer_DrawModoSimonClasico(uint8_t paso_j1, uint8_t paso_j2) {
    ...
    /* Divisor central de la mesa */
    ILI9341_FillRect(0, COCK_DIV_Y, COCK_ZONE_W, COCK_DIV_H, COLOR_DARKGRAY);

    uint8_t activo[2] = { paso_j1, paso_j2 };

    /* Cuadricula 2x2 local: 0=Rojo(arriba-izq) 1=Verde(arriba-der)
     *                       2=Azul(abajo-izq)  3=Amarillo(abajo-der) */
    uint16_t cw = 70, ch = 55, gap = 10;
    uint16_t gx0 = (COCK_ZONE_W - (2 * cw + gap)) / 2;
    uint16_t gx1 = (uint16_t)(gx0 + cw + gap);
    uint16_t gy0 = 30;
    uint16_t gy1 = (uint16_t)(gy0 + ch + gap);

    uint16_t cx[4] = { gx0, gx1, gx0, gx1 };
    uint16_t cy[4] = { gy0, gy0, gy1, gy1 };

    for (uint8_t p = 0; p < 2; p++) {
        Cockpit_DrawString(p, 8, 5, (p == 0) ? "J1" : "J2",
                            (p == 0) ? COLOR_P1 : COLOR_P2, COLOR_BLACK, 2);

        for (uint8_t c = 0; c < 4; c++) {
            uint16_t col = (activo[p] == c) ? NOTE_COLOR[c] : LANE_COLOR[c];
            Cockpit_FillRect(p, cx[c], cy[c], cw, ch, col);
        }
    }
}
...
void Renderer_DrawModoSimonJoystick(uint8_t paso_j1, uint8_t paso_j2) {
    ...
    draw_player_topbar(P1_X_OFF, COLOR_P1, "J1");
    draw_player_topbar(P2_X_OFF, COLOR_P2, "J2");
    ILI9341_FillRect(DIV_X, 0, DIV_W, LCD_H, COLOR_WHITE);

    static const char *const dir_txt[4] = { "ARRIBA", "ABAJO", "IZQ", "DER" };
    static const uint16_t    xoff[2]    = { P1_X_OFF, P2_X_OFF };
    uint8_t activo[2] = { paso_j1, paso_j2 };

    /* Distribucion en cruz (tipo D-pad): 0=Arriba 1=Abajo 2=Izquierda 3=Derecha */
    for (uint8_t p = 0; p < 2; p++) {
        uint16_t xo = xoff[p];
        uint16_t bw = 50, bh = 46;
        uint16_t cxm = (uint16_t)(xo + PLAYER_W / 2 - bw / 2);

        uint16_t bx[4] = { cxm, cxm, (uint16_t)(xo + 8), (uint16_t)(xo + PLAYER_W - bw - 8) };
        uint16_t by[4] = { 40,  150, 95,                  95 };

        for (uint8_t d = 0; d < 4; d++) {
            uint16_t col = (activo[p] == d) ? NOTE_COLOR[d] : LANE_COLOR[d];
            ILI9341_FillRect(bx[d], by[d], bw, bh, col);

            uint16_t tw = str_pixel_w(dir_txt[d], 1);
            draw_string((uint16_t)(bx[d] + (bw - tw) / 2), (uint16_t)(by[d] + bh / 2 - 3),
                        dir_txt[d], COLOR_WHITE, col, 1);
        }
    }
}
