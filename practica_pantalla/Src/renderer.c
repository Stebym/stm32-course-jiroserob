...
static void cockpit_2x2_rect(uint8_t c, uint16_t *lx, uint16_t *ly, uint16_t *lw, uint16_t *lh) {
    static const uint8_t es_derecha[4] = { 0, 1, 0, 1 };
...
static void cockpit_clasico_draw_jugador(uint8_t jugador, uint8_t paso) {
    ...
    Cockpit_DrawString(jugador, 8, 5, (jugador == 0) ? "J1" : "J2",
...
static void cockpit_draw_jugador(uint8_t jugador, uint8_t paso) {
    Cockpit_FillRect(jugador, 0, 0, COCK_ZONE_W, COCK_ZONE_H, COLOR_BLACK);
    Cockpit_DrawString(jugador, 8, 4, (jugador == 0) ? "J1" : "J2",
...
void Renderer_ActualizarCursorJoystick(uint16_t joy_x, uint16_t joy_y, uint8_t listo) {
    uint16_t px = (uint16_t)(SJ1P_CURSOR_X0 + ((uint32_t)joy_x * (SJ1P_CURSOR_X1 - SJ1P_CURSOR_X0)) / 4095u);
    uint16_t py = (uint16_t)(SJ1P_CURSOR_Y0 + ((uint32_t)joy_y * (SJ1P_CURSOR_Y1 - SJ1P_CURSOR_Y0)) / 4095u);
